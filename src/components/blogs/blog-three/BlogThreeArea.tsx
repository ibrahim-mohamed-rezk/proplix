...
const BlogThreeArea = async () => {
  ...
  // featch types
  const feachTypes = async () => {
    try {
      const response = await getData(`types`, {}, {});
            ...
            <div key={item.id} className="col-lg-6">
              <div className="isotop-item villa sale">
                <article className="blog-meta-one mb-70 lg-mb-40">
                  <figure
                    className={`post-img border-25 position-relative m0 ${item.cover}`}
                  >
                    <Link
