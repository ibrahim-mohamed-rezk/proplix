...
export const MainTab: React.FC<MainTabProps> = ({ property ,token}) => {
  ...
  return (
    <div className="mb-4">
        ...
        <div className="d-flex justify-content-between align-items-start mb-3">
          <div>
            ...
            <p className="text-muted mb-2">
              {t("area")}: {property?.data?.area?.description?.en?.name}
            </p>
            <div className="d-flex align-items-center gap-3 text-sm">
              <span className="text-muted">
                {t("approval_status")}: {property?.data?.approval_status}
              </span>
              <span className="text-muted">
                {t("status")}: {property?.data?.status}
              </span>
            </div>
          </div>
          <div className="text-right">
            ...
            {property?.data?.down_price && (
              <div className="text-muted">
                {t("down_price")}: ${property?.data?.down_price?.toLocaleString()}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="mb-4">
        ...
        <div 
          className="w-100 p-3 border rounded bg-light border-muted text-dark min-h-100"
      ...
      <div className="mb-4">
        ...
        <div className="border rounded p-3 bg-light border-muted">
