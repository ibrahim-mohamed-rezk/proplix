...
const UserAvatarSetting: React.FC<UserAvatarSettingProps> = ({
  ...
  const [tradeLicenceImagePreview, setTradeLicenceImagePreview] =
    useState<string>("");
  ...
  const handleInputChange = (
    ...
    } else if (name === "secondPhone" && value.trim()) {
      const validation = validatePhoneNumber(value, false);
      if (!validation.isValid) {
        setSecondPhoneError(validation.error || "Invalid phone number");
      } else {
        ...
        <div className="col-sm-6">
          <div className="dash-input-wrapper mb-30">
            ...
            <input
              ...
              onChange={handleInputChange}
              placeholder="01234567890"
              className={secondPhoneError ? "is-invalid" : ""}
            />
            {secondPhoneError && (
              <div className="invalid-feedback d-block">{secondPhoneError}</div>
            )}
          </div>
        </div>
        <div className="col-sm-6">
